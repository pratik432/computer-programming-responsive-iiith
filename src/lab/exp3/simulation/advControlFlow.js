...
window.view = {
    ...
    startBtn: function() {
		...
		var inputValue = document.getElementById('simpleLoopInput').value
		if (selected_loop === 'for' && inputValue !== '' && inputValue < 21 && !isNaN(model.inp) )
		{
			this.displayLoop('forLoopContent', 'codeContentFor1')
		}
		if (selected_loop === 'while' && inputValue !== '' && inputValue < 21 && !isNaN(model.inp))
		{
			this.displayLoop('whileLoopContent', 'codeContentWhile1')
		}
		if (selected_loop === 'do-while' && inputValue !== ''&& inputValue < 21 && !isNaN(model.inp))
		{
